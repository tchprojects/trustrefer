...
export function ShareButton() {
  ...
  useEffect(() => {
    ...
    if (showMenu) document.addEventListener("keydown", handle);
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      {showMenu && (
        <div className="fixed left-1/2 top-16 z-[100] mt-2 w-72 -translate-x-1/2 rounded-2xl border border-white/10 bg-[#0f0f0f] p-4 shadow-xl">
          {/* Close button */}
          <button
            onClick={() => setShowMenu(false)}
            aria-label="Close"
            className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-white/10 text-white hover:bg-white/20"
          >
            <X size={12} />
          </button>
          ...
          <div className="flex justify-between">
            {options.map((opt) => (
              <button
                key={opt.label}
                onClick={() => { opt.action(shareUrl); setShowMenu(false); }}
              ...
              >
                ...
                <span className="w-10 text-center text-[9px] leading-tight text-[#666] group-hover:text-white">
                  {opt.label}
                </span>
              </button>
            ))}
          ...
          <button
            onClick={copyLink}
            className="mt-4 flex w-full items-center justify-between rounded-xl border border-white/10 bg-[#141414] px-3 py-2.5 transition-colors hover:border-white/20"
          >
            <span className="truncate text-xs text-[#888]">{shareUrl}</span>
            <span className="ml-2 flex shrink-0 items-center gap-1.5 text-xs font-medium text-white">
...
function EmailIcon() {
  return (
    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1f1f1f] border border-white/10">
